...
DataTable1.onclick = function (event) {
  ...
  NSB.MsgBox(
    "Click on " + row + ", " + col + ". Value is '" + data1[row][col] + "'."
  );
};
...
Button4.onclick = function () {
  ...
  $(table.rows().nodes()).removeClass("highlight");
  $(table.cells().nodes()).removeClass("highlight");
  $(table.rows(2).nodes()[0].cells).addClass("highlight");
};

Switch1.onchange = function () {
  DataTable1.settings.ordering = Switch1.value;
  updateTable();
};

Switch2.onchange = function () {
  DataTable1.settings.paging = Switch2.value;
  updateTable();
};

Switch3.onchange = function () {
  DataTable1.settings.searching = Switch3.value;
  updateTable();
};
