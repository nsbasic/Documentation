...
NSB.bs.addProperties = function(ctrl, type){
    ...
    if (type != "DataTable_bs4" && type != "Breadcrumbs_bs4" && type != "Progressbar_bs4" && type != "Dropdown_bs4") {
    ...
    switch (type) {
    case "Alert_bs4":         NSB.bs.alert.init(ctrl); break;
    case "Breadcrumbs_bs4":   NSB.bs.breadcrumbs.init(ctrl); break;
    case "Button_bs4":        NSB.bs.button.init(ctrl); break;
    case "Checkbox_bs4":      NSB.bs.checkbox.init(ctrl); break;
    case "DataTable_bs4":     NSB.bs.datatable.init(ctrl); break;
    case "Dropdown_bs4":      NSB.bs.dropdown.init(ctrl); break;
    case "Fliptoggle_bs4":    NSB.bs.fliptoggle.init(ctrl); break;
    case "GridColumn_bs4":    NSB.bs.gridcolumn.init(ctrl); break;
    case "Hamburger_bs4":     NSB.bs.dropdown.init(ctrl); break; //Added by BT
    case "Heading_bs4":       NSB.bs.define.text(ctrl, 'value', ''); break;
    case "Input_bs4":         NSB.bs.input.init(ctrl); break;
    case "Jumbotron_bs4":     NSB.bs.jumbotron.init(ctrl); break;
    case "Label_bs4":         NSB.bs.label.init(ctrl); break;
    case "Listgroup_bs4":     NSB.bs.listgroup.init(ctrl); break;
    case "Media_bs4":         NSB.bs.define.src(ctrl); break;
    case "Modal_bs4":         NSB.bs.modal.init(ctrl); break;
    case "Navs_bs4":          NSB.bs.navs.init(ctrl); break;
    case "Pagination_bs4":    NSB.bs.pagination.init(ctrl); break;
    case "Panel_bs4":         NSB.bs.panel.init(ctrl); break;
    case "Progressbar_bs4":   NSB.bs.progressbar.init(ctrl); break;
    case "Radiobutton_bs4":   NSB.bs.radiobutton.init(ctrl); break;
    case "Select_bs4":        NSB.bs.select.init(ctrl); break;
    case "Textarea_bs4":      NSB.bs.input.init(ctrl); break;
    case "Thumbnail_bs4":     NSB.bs.define.src(ctrl); break;
    }
};
    ...
    getHTML: function(id, i, item, type, inline) {
        ...
        const s = `
            <div class='form-check${(inline) ? ' form-check-inline' : ''}'>
              <input id=${id}_${i} class='form-check-input' type='checkbox'${type} value='${item}'>
...
NSB.bs.dropdown = {
    ...
    make: function (id, value, dropdown, klass, disabled, style, styleb, buttonStyle, outline, size, icon,
        ...
        if (icon == 'caret') {
            ...
            s += "    <span class='oi oi-" + icon + "' title='" + iconTitle + "' data-i18n='" + iconTitle + "'></span>\n";
        }
        s += "  </button>\n";
...
NSB.bs.listgroup = {
    ...
    make: function (id, style, klass, disabled, items, itemBadges, appearances, listStyle, bsPopover, icons) {
    ...
    getHTML: function(id, i, item, type, badge, appearance, listStyle, icon) {
        ...
        if (badge !== '')
          badge = "&nbsp;<span id='" + id + "_" + i + "_badge' class='badge badge-primary badge-pill'>" + badge+ "</span>";
        idprop = "id='" + id + "_" + i + "'";  
        ...
        ik = (icon!=='') ? "&nbsp;<span class='oi oi-" + icon + " form-control-feedback' aria-hidden='true'></span>" : "";
...
NSB.bs.navs = {
    ...
    getHTML: function(id, i, item, type, color) {
        ...
        i18n = ` data-i18n='${item}'`;
        s += `<a class='nav-link${type}'${c} href='#' ${i18n}>${item}</a></li>\n`;
        return s;
        },
    ...
    addItem: function(item, active, disabled) {
        ...
        var s = NSB.bs.navs.getHTML(this.id, len, item, type);
...
NSB.bs.radiobutton = {
    ...
    getHTML: function(id, i, item, type, inline) {
        ...
        const s = `
            <div class='form-check${(inline) ? ' form-check-inline' : ''}'>
              <input id=${id}_${i} class='form-check-input' type='radio'${type} value='${item}' name=${id}>
              <label class='form-check-label' data-i18n='${item}'>${item}</label>
            </div>`;
        return s;
        },
...
NSB.bs.select = {
    ...
    make: function (id, style, disabled, items, values, multiSelect, size, name, required) {
        ...
        s = "<select id='" + id + "_contents' class='form-control'" + disabled + multiSelect + size + contentStyle + required + " name=" + name + ">\n";
        for (i = 0; i < arrItems.length; i++) {
           item = arrItems[i];
...
NSB.bs.define = {
    ...
    header_control_label: function(ctrl){
        NSB.defineProperty(ctrl, 'header', {
            set: function (n) {
                if ($('#'+ctrl.id+' .bs4-control-label').length === 0)
                    $(ctrl).prepend("<label class='bs4-control-label' for=" + ctrl.id + "></label>");
                $('#'+ctrl.id+' .bs4-control-label').text(n);
            },
            get: function () {
                return $('#'+ctrl.id+' .bs4-control-label').text();
            }
        });
    },
    ...
    footer_help_block: function(ctrl){
        NSB.defineProperty(ctrl, 'footer', {
            set: function (n) {
                if ($('#'+ctrl.id+' .bs4-help-block').length === 0)
                    $(ctrl).append("<span class='bs4-help-block'></span>");
                $('#'+ctrl.id+' .bs4-help-block').text(n);
            },
            get: function () {
                return $('#'+ctrl.id+' .bs4-help-block').text();
            }
        });
    },
