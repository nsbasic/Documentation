"use strict";function _typeof(o){"@babel/helpers - typeof";return _typeof="function"==typeof Symbol&&"symbol"==typeof Symbol.iterator?function(o){return typeof o}:function(o){return o&&"function"==typeof Symbol&&o.constructor===Symbol&&o!==Symbol.prototype?"symbol":typeof o},_typeof(o)}/**************************************************************************************/// Bootstrap 5 functions
// (c) 2026 NSB Corporation. All rights reserved.
var NSB=NSB||{};// setup the NSB namespace, if needed
NSB.bs={};NSB.overlayShowing=[];NSB.bs.addProperties=function(ctrl,type){if(type!="DataTable_bs5"&&type!="Breadcrumbs_bs5"&&type!="Progressbar_bs5"&&type!="Dropdown_bs5"&&type!="Button_bs5"){$("#"+ctrl.id).addClass("collapse show");ctrl.collapse=function(arg){if(!arg)arg=$("#"+ctrl.id).hasClass("in")?"hide":"show";$("#"+ctrl.id).collapse(arg)}};ctrl.hide=function(options){// if the BS control has an overlay (popover, dropdown, etc.), BS calls
...
if(NSB.overlayShowing[ctrl.id]){NSB.overlayShowing[ctrl.id]=false}else{$("#"+ctrl.id).hide(options)}};$("[data-bs-toggle=\"popover\"]").on("show.bs.popover",function(e){NSB.overlayShowing[e.target.id]=true});$("[data-bs-toggle=\"tooltip\"]").on("show.bs.tooltip",function(e){NSB.overlayShowing[e.target.id]=true});ctrl.show=function(options){$("#"+ctrl.id).show(options)};ctrl.toggle=function(options){if($(ctrl).hasClass("modal"))$(ctrl).modal("toggle");else if($(ctrl).hasClass("offcanvas"))$(ctrl).offcanvas("toggle");else $("#"+ctrl.id).toggle(options)};NSB.bs.define.badge(ctrl,type);ctrl.fadeIn=function(a,c,d){$("#"+ctrl.id).fadeIn(a,c,d)};ctrl.fadeOut=function(a,c,d){$("#"+ctrl.id).fadeOut(a,c,d)};ctrl.slideUp=function(a,c,d){$("#"+ctrl.id).slideUp(a,c,d)};ctrl.slideDown=function(a,c,d){$("#"+ctrl.id).slideDown(a,c,d)};NSB.addProperties.addChild(ctrl);NSB.addProperties.bounds(ctrl);switch(type){case"Alert_bs5":NSB.bs.alert.init(ctrl);break;case"Breadcrumbs_bs5":NSB.bs.breadcrumbs.init(ctrl);break;case"Button_bs5":NSB.bs.button.init(ctrl);break;case"Carousel_bs5":NSB.bs.carousel.init(ctrl);break;case"Checkbox_bs5":NSB.bs.checkbox.init(ctrl);break;case"DataTable_bs5":NSB.bs.datatable.init(ctrl);break;case"Dropdown_bs5":NSB.bs.dropdown.init(ctrl);break;case"Fliptoggle_bs5":NSB.bs.fliptoggle.init(ctrl);break;case"GridColumn_bs5":NSB.bs.gridcolumn.init(ctrl);break;case"Hamburger_bs5":NSB.bs.dropdown.init(ctrl);break;//Added by BT
case"Heading_bs5":NSB.bs.define.text(ctrl,"value","");break;case"Input_bs5":NSB.bs.inputItem.init(ctrl);break;case"Input-group_bs5":NSB.bs.inputGroup.init(ctrl);break;case"Input-item_bs5":NSB.bs.inputItem.init(ctrl);break;case"Jumbotron_bs5":NSB.bs.jumbotron.init(ctrl);break;case"Label_bs5":NSB.bs.label.init(ctrl);break;case"Listgroup_bs5":NSB.bs.listgroup.init(ctrl);break;case"Media_bs5":NSB.bs.define.src(ctrl);break;case"Modal_bs5":NSB.bs.modal.init(ctrl);break;case"Navs_bs5":NSB.bs.navs.init(ctrl);break;case"OffCanvas_bs5":NSB.bs.offcanvas.init(ctrl);break;case"Pagination_bs5":NSB.bs.pagination.init(ctrl);break;case"Panel_bs5":NSB.bs.panel.init(ctrl);break;case"Progressbar_bs5":NSB.bs.progressbar.init(ctrl);break;case"Radiobutton_bs5":NSB.bs.radiobutton.init(ctrl);break;case"Select_bs5":NSB.bs.select.init(ctrl);break;case"Textarea_bs5":NSB.bs.textarea.init(ctrl);break;case"Thumbnail_bs5":NSB.bs.define.src(ctrl);break}};NSB.bs.alert={init:function init(ctrl){NSB.bs.define.appearance(ctrl);NSB.defineProperty(ctrl,"value",{set:function set(value){ctrl.childNodes[2].data=value},get:function get(){return ctrl.childNodes[2].data}})}};NSB.bs.breadcrumbs={init:function init(ctrl){NSB.bs.define.length(ctrl);NSB.bs.define.disabled_contents(ctrl);ctrl.setValue=function(arrItems){var i,s,onclick,i18n;if(arrItems==="")arrItems=[];s=onclick="";for(i=0;i<arrItems.length;i++){onclick=" onclick='if (!this.disabled) ".concat(this.id,".onclick(this.textContent);'");i18n=" data-i18n='".concat(arrItems[i],"'");if(i!=arrItems.length-1&&!this.disabled){s+="  <li id=".concat(this.id,"_").concat(i," class='breadcrumb-item'").concat(onclick).concat(i18n," style='cursor:pointer'><a>").concat(arrItems[i],"</a></li>\n")}else{s+="  <li id=".concat(this.id,"_").concat(i," class='active breadcrumb-item'").concat(onclick).concat(i18n," style='cursor:not-allowed'>").concat(arrItems[i],"</li>\n")}}// console.log(s);
this.innerHTML=s}}};NSB.bs.button={init:function init(ctrl){NSB.bs.define.appearance(ctrl);NSB.bs.define.text(ctrl,"value","")}};NSB.bs.carousel={make:function make(id,style,klass,autoplay,buttons,captions,crossfade,descripions,images,indicators,interval){var i,s,arrCaptions,arrDescriptions,arrImages,items,ride;arrCaptions=captions.trim().split("\n");arrDescriptions=descripions.trim().split("\n");arrImages=images.trim().split("\n");items=Math.max(arrCaptions.length,arrDescriptions.length,arrImages.length);if(klass!=="")klass=" "+klass;if(crossfade)klass+=" carousel-fade";ride="";if(autoplay!=="")ride="data-bs-ride=\"".concat(autoplay,"\"");s="<div id=".concat(id," class=\"carousel slide\" style=\"").concat(style,"\" ").concat(ride,">\n");// indicators div
if(indicators){s+="  <div class=\"carousel-indicators\">\n";for(i=0;i<items;i++){if(i===0){s+="    <button type=\"button\" data-bs-target=#".concat(id," data-bs-slide-to=\"0\" class=\"active\" ></button>\n")}else{s+="    <button type=\"button\" data-bs-target=#".concat(id," data-bs-slide-to=\"").concat(i,"\" ></button>\n")}}s+="  </div>\n"}// images div
s+="  <div class=\"carousel-inner\">\n";for(i=0;i<items;i++){var caption=arrCaptions[i];var description=arrDescriptions[i];var image=arrImages[i];s+=this.getSlideHTML(id,i,caption,description,image,interval)}s+="  </div>\n";// buttons div
if(buttons){s+="  <button class=\"carousel-control-prev\" type=\"button\" data-bs-target=#".concat(id," data-bs-slide=\"prev\">\n    <span class=\"carousel-control-prev-icon\" ></span>\n    <span class=\"visually-hidden\">Previous</span>\n  </button>\n  <button class=\"carousel-control-next\" type=\"button\" data-bs-target=#").concat(id," data-bs-slide=\"next\">\n    <span class=\"carousel-control-next-icon\" ></span>\n    <span class=\"visually-hidden\">Next</span>\n  </button>\n")}s+="</div>\n";// console.log(s);
return s},init:function init(ctrl){ctrl.value=-1;NSB.bs.define.length(ctrl);NSB.bs.define.disabled_contents(ctrl)},getSlideHTML:function getSlideHTML(id,i,caption,description,image,interval){var s;var active="";if(i===0)active="active";s="    <div class=\"carousel-item ".concat(active,"\" data-bs-interval=").concat(interval,">\n");if(image)s+="      <img id=".concat(id,"_image").concat(i," src=\"").concat(image,"\" class=\"d-block w-100\" >\n");if(caption){s+="      <div class=\"carousel-caption d-none d-md-block\">\n";s+="         <h5>".concat(caption,"</h5>\n");if(description){s+="         <p>".concat(description,"</p>\n")}s+="      </div>\n"}s+="    </div>\n";return s}};NSB.bs.checkbox={make:function make(id,style,disabled,items,inline,value){var i,s,arrItems,item,type;arrItems=items.split("\n");if(items===""){arrItems=[]}s="  <div id='"+id+"_contents'>";for(i=0;i<arrItems.length;i++){item=arrItems[i];type="";if(item.substr(0,1)=="*")type="disabled";if(item.substr(0,1)==">")type="checked";if("*>".indexOf(item.substr(0,1))>=0)item=item.substr(1);if(i===value)type="checked";if(disabled!=="")type="disabled";s+=this.getHTML(id,i,item,type,inline)}s+="\n  </div>\n";// console.log(s);
...
ctrl.addEventListener("onclick",function(ctrl){return ctrl});ctrl.clear=function(){$("#"+this.id+"_list").empty()};NSB.defineProperty(ctrl,"value",{set:function set(n){if(n>=$("[id^="+ctrl.id+"_"+"] a").length)return;$("[id^="+ctrl.id+"_"+"] a").removeClass("active");if(n>=0)$("#"+ctrl.id+"_"+n+" a").addClass("active")},get:function get(){for(var i=0;i<NSB.$(ctrl.id+"_list").children.length;i++){if($("#"+ctrl.id+"_"+i+" a").hasClass("active"))return i}return-1}})},getHTML:function getHTML(id,i,item,link,type,color){var onclick,idprop,s,c,i18n;if(type!=="")type=" "+type;onclick="onclick=\"event.stopPropagation(); if (!$('#".concat(id,"_").concat(i," a').hasClass('disabled')) {").concat(id,".value=").concat(i,";").concat(id,".onclick(").concat(i,");}\"");idprop="id='".concat(id,"_").concat(i,"'");s="  <li ".concat(idprop," class='nav-item' ").concat(onclick," style='line-height:24px; cursor:pointer'>");c=color==""?"":" style='color:".concat(color,"'");i18n=" data-i18n='".concat(item,"'");s+="<a class='nav-link".concat(type,"'").concat(c," href='").concat(link,"' ").concat(i18n,">").concat(item,"</a></li>\n");return s},addItem:function addItem(item,link,active,disabled){var type="";var len=0;if(active)type="active";if(disabled)type="disabled";if(NSB.$(this.id+"_list"))len=NSB.$(this.id+"_list").children.length;var s=NSB.bs.navs.getHTML(this.id,len,item,link,type);NSB.$(this.id+"_list").insertAdjacentHTML("beforeend",s);//console.log(s);
...
return this.children[this.value].innerText}};NSB.bs.offcanvas={init:function init(ctrl){$(ctrl).removeClass("collapse show");NSB.defineProperty(ctrl,"header",{set:function set(n){if($("#"+ctrl.id+" .offcanvas-title").length===0)$(ctrl).prepend("<h4 class='offcanvas-title'><span id="+ctrl.id+"_badge class=badge></span></h4>");$("#"+ctrl.id+" .offcanvas-title").html(n)},get:function get(){return $("#"+ctrl.id+" .offcanvas-title").html()}});NSB.defineProperty(ctrl,"value",{set:function set(n){if($("#"+ctrl.id+" .offcanvas-body").length===0)$(ctrl).append("<div class='offcanvas-body'></div>");$("#"+ctrl.id+" .offcanvas-body").html(n)},get:function get(){return $("#"+ctrl.id+" .offcanvas-body").html()}});NSB.defineProperty(ctrl,"footer",{set:function set(n){if($("#"+ctrl.id+" .offcanvas-footer").length===0)$(ctrl).append("<div class='offcanvas-footer'></div>");$("#"+ctrl.id+" .offcanvas-footer").html(n)},get:function get(){return $("#"+ctrl.id+" .offcanvas-footer").html()}})}};NSB.bs.pagination={make:function make(id,disabled,items,size){var i,s,k,arrItems,item,onclick,idprop;items="&laquo;\n"+items+"\n&raquo;";arrItems=items.split("\n");if(items==="")arrItems=[];s="";for(i=0;i<arrItems.length;i++){item=arrItems[i];k="pagination"+size;if(item.substr(0,1)==">"){k+=" active";item=item.substr(1)}if(disabled!==""&&item.substr(0,1)!=="*")item="*"+item;if(item.substr(0,1)=="*"){k+=" disabled";item=item.substr(1)}s+=this.getHTML(id,i,item,k)}//console.log(s);
